...
export default function Blogs() {
	const [equipments, equipmentsLoading, equipmentsError] = useCollection(collection(db, "equipments"));
    const [blogs, blogsLoading, blogsError] = useCollection(collection(db, "blogs"));

    if (equipmentsLoading || blogsLoading) {
        return (<div>Loading...</div>);
    } else if (equipmentsError || blogsError) {
        return (<div>Error. Please refresh.</div>);
    }

    let equipmentsData = equipments.docs.map(doc => doc.data());
    let jsx = equipmentsData.map((val) => {
		  ...
		  <div className="w-[100%] h-[15em] relative">
			...
			<div className="min-h-[20em] grid grid-cols-2fr_1fr absolute top-0 left-0">
			  <div className="flex flex-col m-[150px] items-left justify-center gap-[4px]">
				<p className="text-blue-950 text-[44px]">Our Equipment</p>
